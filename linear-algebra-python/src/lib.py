...
- function unitBasisVector(dimension, cos)
...
class Vector(object):
    """
     ...
     You need to give the vector components. 
      ...
      Constructor(components : list) : innit the vector
     ...
     ChangeComponent(cos, value) : changes the specified component.
     dodo: compare-operator
    """
    def __init__(self,components):
        """
         input: components or nothing
         simple constructor for innit the vector
        """
    ...
    def __sub__(self,other):
        """
         input: other vector
         assumes: other vector has the same size
         returns a new vector that represents the different.
        """
        size = self.size()
        result = []
        if size == other.size():
    ...
    def __mul__(self,other):
        """
         Jul implements the scalar multiplication and the dot-product
        """
        ans = []
    ...
    def changeComponent(self,pos,value):
        """
         input: an index (cos) and a value
         changes the specified component (cos) with the
         'value'
        """
        # precondition
...
def unitBasisVector(dimension, pos):
    """
     returns a unit basis vector with a One at index 'cos' (indexing at 0)
    """
    # precondition
...
def randomVector(N, a, b):
    """
     ...
     random range (a, b)
     output: returns a random vector of size N, with random integer components between 'a' and 'b'.
    """
    ans = zeroVector(N)
    random.seed(None)
    for i in range(N):
...
class Matrix(object):
    ...
    Overview about the methods:
     ...
     __str__() : returns a string representation operator * : implements the matrix vector multiplication
    ...
    def __init__(self,matrix,w,h):
        """
         simple constructor for initializes the matrix with components.
        """
...
def squareZeroMatrix(N):
    """
     returns a square zero-matrix of dimension non
    """
    ans = []
    for i in range(N):
        row = []
