...
class CNN():
    ...
    def __init__(self, conv1_get, size_p1, bp_num1, bp_num2, bp_num3, rate_w=0.2, rate_t=0.2):
        ...
        self.w_conv1 = [np.mat(-1*np.random.rand(self.conv1[0], self.conv1[0])+0.5)
    ...
    def convolute(self, data, convs, w_convs, thre_convs, conv_step):
        ...
        for i_focus in range(0, size_data - size_conv + 1, conv_step):
            for j_focus in range(0, size_data - size_conv + 1, conv_step):
                focus = data[i_focus:i_focus + size_conv,
                             j_focus:j_focus + size_conv]
        ...
        for i_map in range(num_conv):
            ...
            for i_focus in range(len(data_focus)):
                net_focus = np.sum(np.multiply(
                    data_focus[i_focus], w_convs[i_map])) - thre_convs[i_map]
        ...
        for i_map in range(len(featuremaps)):
            ...
            for i_focus in range(0, size_map, size_pooling):
                for j_focus in range(0, size_map, size_pooling):
                    focus = map[i_focus:i_focus + size_pooling,
        ...
        for i_map in range(num_map):
            ...
            for i in range(0, size_map, size_pooling):
                for j in range(0, size_map, size_pooling):
                    ...
                    i_pool = i_pool + 1
            pd_conv2 = np.multiply(pd_conv1, np.multiply(
                out_map[i_map], (1-out_map[i_map])))
            ...
            for p in range(len(datas_train)):
               ...
               '''
                ...
                for k_conv in range(self.conv1[1]):
                    ...
                    self.thre_conv1[k_conv] = self.thre_conv1[k_conv] - \
